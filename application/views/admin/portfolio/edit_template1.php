...
<div class="old-html" style="opacity: 0"><?php echo $portfolio['html'] ?></div>
    ...
    <script>
        ...
        $(document).ready(function() {
          setTimeout(function(args) {
            $('.demo').empty()
            var old_html = $('.old-html').html()
            old_html = old_html.split('</section>');
            old_html[0] = old_html[0].replace('<div>', '')
            //delete old_html.removeItem[old_html.length - 1];
            for (var i = 0; i < old_html.length; i++) {
              var html = '<div class="lyrow ui-draggable"><a href="#close" class="remove label label-important"><i class="icon-remove icon-white"></i>Remove</a> <span class="drag label"><i class="icon-move"></i>Drag</span><div class="preview"></div><div class="view">';
              html += old_html[i];
              html += '</div></div>';
              //old_html[i] = old_html[i].replace('<section ', '<section contenteditable="true"')
              old_html[i] = html
            }
            console.log(old_html)
            $('.demo').html(old_html);
            $('.demo section').attr('contenteditable', true)
            $('.lyrow.ui-draggable').last().remove()
          }, 300)
        })
    </script>
</body>
